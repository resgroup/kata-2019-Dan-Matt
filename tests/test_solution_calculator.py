...
def test_should_return_RROO_when_2x_full_1_hours_remainder(testtime = '12:56:01'):
    clock = berlin_clock.light_calculator.BerlinClock(testtime)
    expected_solution = clock.calculate_1_hour_lights()
    assert expected_solution == 'RROO'

def test_should_return_YYYYYYYYYYY_when_11x_5_minutes(testtime = '12:56:01'):
    clock = berlin_clock.light_calculator.BerlinClock(testtime)
    expected_solution = clock.calculate_5_minute_lights()
    assert expected_solution == 'YYYYYYYYYYY'


def test_should_return_YOOO_when_1x_1_minutes_remainder(testtime = '12:56:01'):
    clock = berlin_clock.light_calculator.BerlinClock(testtime)
